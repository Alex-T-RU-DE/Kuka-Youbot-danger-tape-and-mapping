...
class map_summ
{
 ...
 public:
  ...
  void update_t_map(const nav_msgs::OccupancyGrid msg)
	{   int arr_size=static_cast<int>(sum_map.info.width)*static_cast<int>(sum_map.info.height);
		
		if(arr_size==static_cast<int>(msg.info.width)*static_cast<int>(msg.info.height))
		 { 
		  for (int i=0; i<arr_size; i++)
			{  
				if(static_cast<int>(msg.data[i])==100)
					{	
						//cout << "found point in [ "<< i << "]"<< endl;
						sum_map.data[i]=100;
					}
			}
			...
			cout<< "Error: sizes are different (" << arr_size << " and "<< static_cast<int>(msg.info.width)*static_cast<int>(msg.info.height) << ")" << endl;
